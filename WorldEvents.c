...
#pragma warning - write a response for the Haunted Mall
  SimpleObject responseTwo;
...
WorldEvent *elderThree() {
  ...
  SimpleObject responseFour;
  char* look = malloc(sizeof(char[200]));
  strcpy(look, "ACTION: ");
  strcat(look, ACTION_LOOKAROUND);
  memccpy(responseFour.displayText, look, 0, sizeof(char[48]));
...
WorldEvent *castleThirteen() {
  ...
  SimpleObject responseTwo;
    memccpy(responseTwo.displayText, "Let's be honest: I know nothing. This whole Squirrel War thing might be a McGuffin as far as I know.", 0, sizeof(char[128]));
...
WorldEvent *castleTwenty() {
  ...
  SimpleObject responseTwo;
  memccpy(responseTwo.displayText, "Okay. then let's go on! Find new world, explore new heights! I am ready!", 0, sizeof(char[128]));
